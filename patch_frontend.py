...
# 1. Add Auto Compliance Retrieval Panel HTML
auto_panel_html = """
                <!-- ADDED: Auto Compliance Retrieval Result Panel -->
                <div id="compliance-result-panel" class="mt-6 hidden bg-white rounded-xl p-6 border border-gray-200 shadow-sm transition-opacity duration-300">
                    <h3 id="lbl-compliance-title" class="text-lg font-bold mb-4 text-gray-900 border-b pb-2 border-gray-200">Compliance Policy Match</h3>
                    <div class="mb-4">
                        <span id="lbl-compliance-ctx" class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Compliance Context</span>
                        <p id="res-compliance-ctx" class="text-sm text-gray-800 whitespace-pre-wrap bg-gray-50 p-4 rounded border border-gray-100">-</p>
                    </div>
                    <div>
                        <span id="lbl-retrieved-regs" class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Retrieved Regulations</span>
                        <div id="res-retrieved-regs" class="space-y-3"></div>
                    </div>
                </div>
"""

# Insert after <div class="grid grid-cols-1 md:grid-cols-2 gap-6">...</div></div>
# Which ends just before <!-- Results Section --> closing tags
# The safest way is to insert before </main>
content = content.replace('</main>', auto_panel_html + '            </div>\n        </main>', 1)
content = content.replace('</div>\n            </div>\n        </main>', '</div>\n        </main>')

# 2. Add Manual Compliance Panel HTML
manual_panel_html = """
        ...
        <section id="manual-compliance-search" class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 mt-8 mb-12">
            ...
            <div id="manual-result-section" class="mt-6 hidden transition-opacity duration-300 opacity-0">
                <div class="bg-gray-50 rounded-xl p-6 border border-gray-200 shadow-sm">
                    <div class="mb-4">
                        <span id="lbl-manual-ctx" class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Compliance Context</span>
                        <p id="manual-res-ctx" class="text-sm text-gray-800 whitespace-pre-wrap bg-white p-4 rounded border border-gray-100">-</p>
                    </div>
                    <div>
                        <span id="lbl-manual-regs" class="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-2">Retrieved Regulations</span>
...
content = content.replace('statusIncomplete: "INCOMPLETE"\n            }', 'statusIncomplete: "INCOMPLETE"' + en_add)
content = content.replace('statusIncomplete: "不完整"\n            }', 'statusIncomplete: "不完整"' + zh_add)
content = content.replace('statusIncomplete: "TIDAK LENGKAP"\n            }', 'statusIncomplete: "TIDAK LENGKAP"' + ms_add)

# 4. Add logic to updateStatusUI and changeLanguage
updateStatusUI_add = """
...
changeLanguage_add = """            document.getElementById('lbl-status').innerText = t.lblStatus;
...
content = content.replace("            document.getElementById('lbl-status').innerText = t.lblStatus;", changeLanguage_add)

updateStatusUI_text_add = """            } else if (statusData === 'AUDITING') {
...
# 5. Append performAutoComplianceRetrieval, manualRetrieve and buildRegulationsHTML
js_add = """
        ...
        function buildRegulationsHTML(regs) {
            ...
            regs.forEach(reg => {
                html += `
                    <div class="bg-white p-4 rounded border border-gray-100 shadow-sm text-sm">
                        <div class="font-bold text-gray-800 mb-1">${reg.title || ''}</div>
                        <div class="text-gray-600 mb-2 line-clamp-2" title="${reg.content}">${reg.content || ''}</div>
                        <div class="flex space-x-3 text-xs">
                            <span class="bg-gray-100 text-gray-600 px-2 py-1 rounded">HS: ${reg.hs_code || 'N/A'}</span>
                            <span class="bg-blue-50 text-blue-600 px-2 py-1 rounded">${t.lblCountry}: ${reg.country || 'N/A'}</span>
                        </div>
                    </div>
                `;
            });
...
content = content.replace('        // Initialize UI with default language on load', js_add + '\n        // Initialize UI with default language on load')

# 6. Append hook to the very end of processInvoice()
# Find the exact end of processInvoice function:
process_invoice_end = """            } finally {
                // Reset UI state
...
content = content.replace(process_invoice_end, process_invoice_hook)
